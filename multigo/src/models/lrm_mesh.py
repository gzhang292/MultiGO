...
class InstantMesh(nn.Module):
    ...
    def forward_geometry(self, planes, render_cameras, render_size=256):
        ...
        # images_input = img_feat.detach().cpu().numpy()*255
        # kiui.write_image(f'./tmp.jpg', images_input.astype(np.uint8))
        ...
        **kwargs,
        ...
        if not use_texture_map:
            ...
            vertices_colors = self.synthesizer.get_texture_prediction(
                planes, vertices_tensor).clamp(0, 1).squeeze(0).cpu().detach().numpy()
            vertices_colors = (vertices_colors * 255).astype(np.uint8)

            return vertices.cpu().detach().numpy(), faces.cpu().detach().numpy(), vertices_colors
