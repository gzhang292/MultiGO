...
def spherical_camera_pose(azimuths: np.ndarray, elevations: np.ndarray, radius=2.5):
    ...
    # ys = - radius * np.sin(elevations)
    # zs = radius * np.cos(elevations) * np.cos(azimuths)
...
def get_zero123plus_input_cameras(batch_size=1, radius=4.0, fov=30.0):
    ...
    render_w2cs = torch.linalg.inv(c2ws)
    ...
    return cameras.unsqueeze(0).repeat(batch_size, 1, 1), render_w2cs.unsqueeze(0)
