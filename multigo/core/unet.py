...
class MVAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if B == 0:
            x = x.reshape(BV, 1, C, H, W).permute(0, 1, 3, 4, 2).reshape(BV, -1, C)
        else:
        ...
        if B == 0:
            x = x.reshape(BV, 1, H, W, C).permute(0, 1, 4, 2, 3).reshape(BV, C, H, W)
        else:
...
class UNet(nn.Module):
    ...
    def forward(self, x, other= None, shape=None):
        # x: [B, Cin, H, W]
        B, Cin, H, W = x.shape
        # first
        x = self.conv_in(x)
        
        x = torch.cat([x.unsqueeze(1), shape], dim=1)
        b, v, c, h, w = x.shape
        x = x.view(b*v, c, h, w)
