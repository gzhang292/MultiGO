...
sys.path.append('./Models/autoencoders/')

from transformers__.perceiver_1d import Perceiver
from transformers__.attention import ResidualCrossAttentionBlock
from utils__.checkpoint import checkpoint
from utils__.base import BaseModule
from utils__.typing import *

from utils_ import AutoEncoder, FourierEmbedder, get_embedder
...
class PerceiverCrossAttentionEncoder(nn.Module):
    def __init__(self,
        ...
        #     self.query = nn.Parameter(torch.randn((1024+1, width)) * 0.02)
    ...
    # def forward(self, pc: torch.FloatTensor, feats = None):
    #     """
...
if __name__ == '__main__':
    '''
        input: SMPL_dict
        {
        'full_body' (optional): (1,24,3,3)    torch:concat([body_pose, global_orient], dim = 0)
        'body_pose': (1,23,3,3)     (1,23,6)
        'global_oriend': (1,1,3,3)  (1,1,6)
        'betas': (1,10)
        'scale'
        }
        Output: Voxel Feature
        Voxel Feature: (1,6,32,32) 
    '''
    #--------------input-----------------
    embed_type = 'fourier'
    num_freqs = 8
    ...
    embedder = get_embedder(embed_type=embed_type, num_freqs=num_freqs, include_pi=include_pi)
    ...
    encoder = PerceiverCrossAttentionEncoder(
        ...
        embed_point_feats=embed_point_feats,
        width=width,
        heads=heads,
    ...
    dict = trimesh.load('/hpc2hdd/home/gzhang292/extra_data/THuman2.1/0000/0000.obj')  
