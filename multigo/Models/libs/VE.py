...
class Residual3D(BaseNetwork):
    def __init__(self, numIn, numOut):
        ...
        self.bn1 = nn.BatchNorm3d(self.numOut, dtype=torch.float32, device='cuda')
        self.conv2 = nn.Conv3d(
            self.numOut, self.numOut, bias=self.with_bias, kernel_size=3, stride=1, padding=1, dtype=torch.float32,device='cuda'
        )
        # self.bn2 = nn.GroupNorm(4, self.numOut)
        self.bn2 = nn.BatchNorm3d(self.numOut, dtype=torch.float32, device='cuda')
        # self.conv3 = nn.Conv3d(
        #     self.numOut, self.numOut, bias=self.with_bias, kernel_size=3, stride=1, padding=1
        # )

        if self.numIn != self.numOut:
            self.conv4 = nn.Conv3d(self.numIn, self.numOut, bias=self.with_bias, kernel_size=1, dtype=torch.float32,device='cuda')
        #self.init_weights()
...
class VolumeEncoder(BaseNetwork):
    ...
    def __init__(self, num_in=3, num_out=32, num_stacks=2):
        ...
        self.bn1 = nn.BatchNorm3d(self.num_inter, dtype=torch.float32, device='cuda')
        self.conv2 = nn.Conv3d(
            self.num_inter,
            self.num_out,
            bias=self.with_bias,
        ...
        self.bn2 = nn.BatchNorm3d(self.num_out, dtype=torch.float32, device='cuda')
